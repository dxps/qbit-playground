...
public class HRService {
	...
	private final Map<Integer, Department> departmentCache = new HashMap<>();
	...
	public void addDepartment(final Callback<Boolean> callback,
		...
		System.out.printf("\n%s | HRService > addDepartment |-> department=%s\n", Instant.now(), department);
		...
		final Callback<Boolean> repoCallback = reactor.callbackBuilder()
				.setCallback(Boolean.class, succeeded -> {
					...
					System.out.printf("%s | HRService > addDepartment |<- department=%s\n", Instant.now(), department);
				})
				.setOnTimeout(() -> {
					// callback.accept(false); One way.
					// callback.onTimeout(); Another way
					callback.onError(new TimeoutException("Timeout while adding department with id=" + departmentId));
				})
				.setOnError(error -> {
	...
	public boolean addEmployee(@PathVariable("departmentId") Integer departmentId,
		...
		final Department department = departmentCache.get(departmentId);
